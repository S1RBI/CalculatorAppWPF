// Вставьте сюда код SupabaseHttpClient.cs
...

    {
...

        {
...

            {
                ...
                throw; // КРИТИЧЕСКИ ВАЖНО: Выбрасываем исключение обратно
            }
...

            {
...

                {
...

                    {
                        ...
                        // Дополнительно сохраняем в локальный файл для fallback
...

                        {
                            ...
                            // Логируем ошибку сохранения, но не прерываем работу

                            MessageBox.Show($"Предупреждение: Не удалось сохранить цены локально: {saveEx.Message}",
...

        {
...

            {
                ...
                // КРИТИЧЕСКИ ВАЖНО: Принудительно сохраняем в локальный файл
...

                {
                    ...
                    System.Diagnostics.Debug.WriteLine("✅ Цены покрытий синхронизированы с локальным файлом");
...

                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"❌ Ошибка синхронизации покрытий: {ex.Message}");
...

    {
...

        {
...

            {
                ...
                System.Diagnostics.Debug.WriteLine("✅ PriceManager: Облачный режим активен");
...

            {
                ...
                System.Diagnostics.Debug.WriteLine($"⚠️ PriceManager: Переход в локальный режим - {ex.Message}");



                // ВАЖНО: Принудительно загружаем локальные данные
...

                {
                    ...
                    System.Diagnostics.Debug.WriteLine("❌ PriceManager: Локальные данные не найдены, инициализируем значения по умолчанию");
                ...
                // КРИТИЧЕСКИ ВАЖНО: Выбрасываем исключение, чтобы MainWindow понял, что это локальный режим
                throw;
